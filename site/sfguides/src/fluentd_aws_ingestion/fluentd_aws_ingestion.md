author: Masaya Yanase
id: fluentd_aws_ingestion
summary: The article details a mechanism for using Fluentd to send system event logs to snowflake as a part of log storing or SIEM. Fluentd is OSS which parses event logs to structured data and put them to S3 bucket.
Please refer to the following article for details about Fluentd
categories: Cybersecurity
environments: web
status: Published 
feedback link: https://github.com/Snowflake-Labs/sfguides/issues
tags: Cybersecurity, SIEM, CSPM, Fluentd, AWS

# Fluentd + Snowflake Integration for Security Analytics

## Overview 
Duration: 5 

The article details a mechanism for using Fluentd to send system event logs to snowflake as a part of log storing or SIEM. Fluentd is OSS which parses event logs to structured data and put them to S3 bucket.
Please refer to the following article for details about Fluentd

https://www.fluentd.org/

Note: Currently there is no plugin which Fluentd can connect to the snowflake internal stage directly.

In this document all of the configuration steps are provided to successfully set up this integration.

### References
- https://docs.hunters.ai/wiki/fluentd
...
- https://cve.mitre.org/cgi-bin/cvekey.cgi?keyword=Apache%20HTTP%20server

### High Level Configuration Steps
- Deploy Apache2 on target server (Amazon Linux)
- Deploy Fluentd agent (td-agent ) on target server
- Setup External Stage on snowflake
- Setup Snowpipe to retrieve data from External Stage
- Prerequisites
- Complete Snowflake Account Creation
- Complete AWS VPC settings like subnet / security rule, etc.
- Complete Bastion Host deployment on AWS VPC
- Complete S3 bucket deployment, create IAM role and get Access Key & Secret Key

### Fluentd + Snowflake Integration Architecture
![A diagram detailing apache logs being ingested into snowflake. This diagram shows an icon of a user with the label 'malicious IP' with an arrow to a cloud icon labeled internet. Internet then connects into a Subnet in an AWS Account to both a bastion host inside of a container labeled server/desktop and a container showing apache and fluentd labeled amazon linux. There is an arrow labeled SSH connecting the bastion host and amazon linux box. An arrow from flientd to apache with the word tail and an arrow with the word put connecting fluentd to an icon laveled AWS s3 external stage. To the right of this digram is another ox called snowflake, an arrow connects snowflake to the AWS s3 external stage and is laeled snowpipe auto ingestion](assets/fluentd-aws-arch.png)


Note: in this scenario, Apache server is facing at internet (anyone can access to it)
